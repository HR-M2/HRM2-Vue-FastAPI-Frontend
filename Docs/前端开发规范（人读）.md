# HRM2 前端开发规范

> 本文档为开发者和 AI 编程助手提供代码编写的约束和建议，确保代码风格和架构一致性。

---

## 目录

1. [代码风格约定](#1-代码风格约定)
2. [Vue 组件规范](#2-vue-组件规范)
3. [Composable 编写规范](#3-composable-编写规范)
4. [类型定义规范](#4-类型定义规范)
5. [组件目录组织](#5-组件目录组织)
6. [API 调用规范](#6-api-调用规范)
7. [样式规范](#7-样式规范)
8. [路由规范](#8-路由规范)
9. [新功能开发流程](#9-新功能开发流程)

---

## 1. 代码风格约定

### 1.1 文件命名规范

| 类型 | 命名风格 | 示例 |
|------|----------|------|
| Vue 组件 | `PascalCase.vue` | `PositionListPanel.vue` |
| Composable | `useCamelCase.ts` | `usePositionManagement.ts` |
| 类型文件 | `camelCase.ts` | `position.ts` |
| 工具函数 | `camelCase.ts` | `formatDate.ts` |
| 路由配置 | `index.ts` | `router/index.ts` |

### 1.2 变量命名规范

| 类型 | 命名风格 | 示例 |
|------|----------|------|
| 响应式变量 | `camelCase` | `positionData`, `isLoading` |
| 常量 | `UPPER_SNAKE_CASE` | `DEFAULT_PAGE_SIZE` |
| 函数 | `camelCase` | `handleSubmit`, `loadPositions` |
| 事件处理 | `handle{Action}` | `handleSelectPosition`, `handleDelete` |
| 布尔值 | `is/has/can/should` 开头 | `isSubmitting`, `hasChanges` |
| 组件 ref | `{name}Ref` | `editFormRef`, `dialogRef` |

### 1.3 注释规范

**文件头注释**（JSDoc 风格）：
```typescript
/**
 * 岗位管理 composable
 * 处理岗位列表加载、选择等操作
 */
```

**函数注释**（简洁描述）：
```typescript
// 计算内容哈希
const calculateHash = async (content: string): Promise<string> => {
  ...
}
```

**模板区域注释**：
```vue
<template>
  <!-- 页面标题 -->
  <div class="page-header">...</div>

  <!-- 主内容区域 -->
  <div class="content-grid">...</div>
</template>
```

### 1.4 导入顺序

按照以下顺序组织导入，每组之间空一行：

```typescript
// 1. Vue 核心
import { ref, reactive, onMounted } from 'vue'

// 2. 第三方库
import { ElMessage } from 'element-plus'
import { Plus, Delete } from '@element-plus/icons-vue'

// 3. 共享组件（@/ 别名）
import { ResumeDetailDialog } from '@/components/common'

// 4. 视图专属组件（相对路径）
import PositionListPanel from './components/PositionListPanel.vue'
import PositionEditForm from './components/PositionEditForm.vue'

// 5. Composables（共享用 @/，视图专属用相对路径）
import { usePositionManagement } from '@/composables/usePositionManagement'
import { usePositionEditor } from './composables/usePositionEditor'

// 6. API
import { getPositions, createPosition } from '@/api/sdk.gen'

// 7. 类型（使用 type 关键字）
import type { PositionListResponse } from '@/api/types.gen'
import type { PositionData } from '@/types'
```

---

## 2. Vue 组件规范

### 2.1 SFC 结构顺序

```vue
<template>
  <!-- 模板内容 -->
</template>

<script setup lang="ts">
// 脚本逻辑
</script>

<style scoped>
/* 样式 */
</style>
```

### 2.2 Script 区块组织

按照以下顺序组织 `<script setup>` 内容：

```typescript
<script setup lang="ts">
// ========== 导入 ==========
import { ref, onMounted } from 'vue'
import { ElMessage } from 'element-plus'
import PositionListPanel from './components/PositionListPanel.vue'
import { usePositionEditor } from './composables/usePositionEditor'
import type { PositionListResponse } from '@/api/types.gen'

// ========== Props & Emits ==========
const props = defineProps<{
  positions: PositionListResponse[]
  loading: boolean
}>()

const emit = defineEmits<{
  select: [pos: PositionListResponse]
  delete: [pos: PositionListResponse]
}>()

// ========== Composables 解构 ==========
const {
  loading,
  positions,
  loadPositions,
  selectPosition
} = usePositionEditor()

// ========== 本地状态 ==========
const dialogVisible = ref(false)
const editFormRef = ref<InstanceType<typeof PositionEditForm>>()

// ========== 计算属性 ==========
const hasSelection = computed(() => selectedId.value !== null)

// ========== 方法 ==========
const handleSelectPosition = (pos: PositionListResponse) => {
  selectPosition(pos)
}

const handleSave = async () => {
  // 业务逻辑
}

// ========== 生命周期 ==========
onMounted(() => {
  loadPositions()
})
</script>
```

### 2.3 Props 定义规范

使用泛型语法定义 Props：

```typescript
// ✅ 推荐：泛型语法
defineProps<{
  positions: PositionListResponse[]
  selectedPositionId: string | null
  loading: boolean
}>()

// ❌ 不推荐：运行时声明
defineProps({
  positions: { type: Array, required: true },
  loading: { type: Boolean, default: false }
})
```

### 2.4 Emits 定义规范

使用泛型语法定义 Emits，参数使用元组：

```typescript
// ✅ 推荐
defineEmits<{
  select: [pos: PositionListResponse]
  delete: [pos: PositionListResponse]
  create: []
  'update:modelValue': [value: string]
}>()

// 在模板中使用
<div @click="$emit('select', pos)">
```

### 2.5 模板规范

**条件渲染顺序**：`v-if` → `v-else-if` → `v-else`

```vue
<div v-if="loading" class="loading">加载中...</div>
<div v-else-if="items.length === 0" class="empty">暂无数据</div>
<div v-else class="list">
  <!-- 列表内容 -->
</div>
```

**多属性换行**：

```vue
<!-- ✅ 推荐：多属性换行 -->
<el-button
  type="primary"
  size="small"
  :loading="submitting"
  @click="handleSubmit"
>
  提交
</el-button>

<!-- ❌ 不推荐：一行过长 -->
<el-button type="primary" size="small" :loading="submitting" @click="handleSubmit">提交</el-button>
```

---

## 3. Composable 编写规范

### 3.1 文件结构

```typescript
/**
 * {功能名称} composable
 * {功能描述}
 */
import { ref, type Ref } from 'vue'
import { ElMessage } from 'element-plus'
import { apiFunction } from '@/api/sdk.gen'
import type { SomeType } from '@/api/types.gen'

export function use{FeatureName}(
  // 可选参数
  someRef?: Ref<SomeType>,
  onSuccess?: (result: SomeType) => void
) {
  // ========== 状态 ==========
  const isLoading = ref(false)
  const data = ref<SomeType | null>(null)

  // ========== 方法 ==========
  const loadData = async () => {
    isLoading.value = true
    try {
      const response = await apiFunction()
      data.value = response.data?.data || null
    } catch (err) {
      console.error('加载失败:', err)
      ElMessage.error('加载失败')
    } finally {
      isLoading.value = false
    }
  }

  const handleAction = async () => {
    // 业务逻辑
    onSuccess?.(result)
  }

  // ========== 返回 ==========
  return {
    // 状态
    isLoading,
    data,
    // 方法
    loadData,
    handleAction
  }
}
```

### 3.2 命名规范

| 场景 | 命名 | 示例 |
|------|------|------|
| 加载状态 | `isLoading` / `loading` | `isLoading.value = true` |
| 提交状态 | `isSubmitting` / `submitting` | `isSubmitting.value = true` |
| 数据加载方法 | `load{Data}` | `loadPositions()` |
| 操作方法 | `handle{Action}` | `handleSubmit()` |
| 工具方法 | 动词开头 | `calculateHash()`, `formatDate()` |

### 3.3 返回值规范

统一返回对象，包含状态和方法：

```typescript
return {
  // 响应式状态
  isLoading,
  isSubmitting,
  data,
  errorMessage,
  
  // 方法
  loadData,
  submitData,
  resetForm,
  
  // 计算属性（如有）
  hasData,
  isValid
}
```

---

## 4. 类型定义规范

### 4.1 类型来源

| 来源 | 路径 | 说明 |
|------|------|------|
| API 自动生成 | `@/api/types.gen` | 后端 API 响应类型，**禁止手动修改** |
| 前端扩展类型 | `@/types/` | 前端特有的类型定义 |

### 4.2 前端类型定义

只有当需要**扩展或组合** API 类型时，才在 `src/types/` 创建：

```typescript
// src/types/position.ts
/**
 * 岗位相关类型定义
 */
import type { ResumeData } from './resume'

/**
 * 岗位数据（前端扩展）
 */
export interface PositionData {
  id: string
  title: string
  department?: string
  // ... 基础字段
  
  // 前端扩展字段
  resume_count?: number
  resumes?: ResumeData[]
}
```

### 4.3 桶文件导出

在 `src/types/index.ts` 统一导出：

```typescript
/**
 * 前端特有类型定义 - 桶文件
 * 
 * 注意：API 自动生成的类型请直接从 '@/api/types.gen' 导入
 */

// 简历相关
export type { ResumeData, ResumeFile } from './resume'

// 岗位相关
export type { PositionData } from './position'

// 任务相关
export type { TaskStatus, ProcessingTask } from './task'
```

### 4.4 类型导入规范

```typescript
// ✅ 使用 type 关键字导入类型
import type { PositionListResponse } from '@/api/types.gen'
import type { PositionData } from '@/types'

// ❌ 不推荐：不使用 type 关键字
import { PositionListResponse } from '@/api/types.gen'
```

---

## 5. 组件目录组织（Co-location 架构）

### 5.1 目录结构

项目采用视图级就近归置（Co-location）架构，视图专属资源与视图文件放在一起：

```
src/
├── components/                    # 跨视图共享组件
│   ├── charts/                    # 图表组件
│   │   ├── AbilityRadarChart.vue
│   │   └── index.ts
│   ├── common/                    # 通用业务组件
│   │   ├── PositionList.vue
│   │   ├── ResumeDetailDialog.vue
│   │   └── index.ts
│   └── layout/                    # 布局组件
│       ├── MainLayout.vue
│       ├── AppSidebar.vue
│       └── AppHeader.vue
├── composables/                   # 跨视图共享 composables
│   ├── usePositionManagement.ts
│   └── useSidebar.ts
└── views/                         # 视图（就近归置）
    └── {view}/
        ├── {View}View.vue         # 视图主文件
        ├── components/            # 视图专属组件（扁平结构）
        │   ├── XxxListPanel.vue
        │   ├── XxxEditForm.vue
        │   └── XxxDialog.vue
        ├── composables/           # 视图专属 composables
        │   └── useXxx.ts
        └── styles/                # 视图专属样式（如需要）
            └── xxx.scss
```

### 5.2 组件命名规范

| 类型 | 命名模式 | 示例 |
|------|----------|------|
| 列表面板 | `{Module}ListPanel.vue` | `PositionListPanel.vue` |
| 编辑表单 | `{Module}EditForm.vue` | `PositionEditForm.vue` |
| 对话框 | `{Module}Dialog.vue` | `PositionJsonDialog.vue` |
| 抽屉 | `{Module}Drawer.vue` | `PositionAIGenerateDrawer.vue` |
| 卡片 | `{Module}Card.vue` | `CandidateAnalysisCard.vue` |

### 5.3 导入规范

**视图专属组件**：使用相对路径直接导入文件，无需 `index.ts` 桶文件：

```typescript
// ✅ 视图专属组件 — 相对路径直接导入
import PositionListPanel from './components/PositionListPanel.vue'
import PositionEditForm from './components/PositionEditForm.vue'
import PositionJsonDialog from './components/PositionJsonDialog.vue'
```

**共享组件**：通过 `index.ts` 桶文件导出，使用 `@/` 别名导入：

```typescript
// ✅ 共享组件 — 从桶文件导入
import { ResumeDetailDialog, PositionList } from '@/components/common'
import { AbilityRadarChart } from '@/components/charts'
```

### 5.4 共享 vs 视图专属的判断

| 场景 | 归属 | 位置 |
|------|------|------|
| 仅在单个视图中使用 | 视图专属 | `views/{view}/components/` |
| 被 2 个及以上视图使用 | 共享 | `components/{category}/` |
| 布局相关（侧边栏、顶栏） | 共享 | `components/layout/` |

---

## 6. API 调用规范

### 6.1 使用生成的 SDK

直接调用 `@/api/sdk.gen` 中的函数：

```typescript
import { getPositions, createPosition, deletePosition } from '@/api/sdk.gen'

// 调用 API
const response = await getPositions({
  query: { page: 1, page_size: 20 }
})

// 访问数据
const items = response.data?.data?.items || []
```

### 6.2 错误处理模式

```typescript
const loadData = async () => {
  isLoading.value = true
  try {
    const response = await getPositions({ query: { page_size: 100 } })
    
    if (response.data?.data?.items) {
      positions.value = response.data.data.items
    }
  } catch (err) {
    console.error('加载岗位列表失败:', err)
    ElMessage.error('加载岗位列表失败')
  } finally {
    isLoading.value = false
  }
}
```

### 6.3 乐观更新模式

对于删除、更新等操作，先更新 UI，失败后回滚：

```typescript
const removeItem = async (item: ItemType) => {
  // 1. 保存原始状态
  const originalItems = [...items.value]
  const originalCount = items.value.length
  
  // 2. 乐观更新 UI
  items.value = items.value.filter(i => i.id !== item.id)
  
  try {
    // 3. 调用 API
    await deleteItem({ path: { id: item.id } })
    ElMessage.success('删除成功')
  } catch (err) {
    // 4. 失败时回滚
    console.error('删除失败:', err)
    ElMessage.error('删除失败')
    items.value = originalItems
  }
}
```

### 6.4 API 客户端重新生成

当后端 API 变更时，运行以下命令重新生成：

```bash
# 确保后端服务运行在 http://localhost:8000
npm run api:gen
```

生成的文件：
- `src/api/sdk.gen.ts` - API 调用函数
- `src/api/types.gen.ts` - TypeScript 类型定义
- `src/api/client.gen.ts` - 客户端配置

> ⚠️ **注意**：生成的文件会被覆盖，**禁止手动修改**。如需封装，在 `src/api/index.ts` 中进行。

---

## 7. 样式规范

### 7.1 scoped 样式

所有组件必须使用 `scoped` 样式隔离：

```vue
<style scoped>
.position-item {
  padding: 12px;
  background: #fafafa;
}
</style>
```

### 7.2 深度选择器

修改 Element Plus 组件内部样式时使用 `:deep()`：

```css
.positions-list-card :deep(.el-card__header) {
  padding: 16px 20px;
  background-color: #fafafa;
}
```

### 7.3 类名命名

使用语义化命名或 BEM 简化版：

```css
/* 语义化命名 */
.page-header { }
.page-title { }
.content-grid { }
.position-item { }
.position-item.active { }

/* 状态修饰 */
.position-item:hover { }
.position-item.active { }
.position-item.disabled { }
```

### 7.4 响应式设计

使用媒体查询处理响应式：

```css
.content-grid {
  display: grid;
  grid-template-columns: 320px 1fr;
  gap: 24px;
}

@media (max-width: 1000px) {
  .content-grid {
    grid-template-columns: 1fr;
  }
}
```

### 7.5 Element Plus 优先

优先使用 Element Plus 组件和样式变量，避免重复造轮子：

```vue
<!-- ✅ 使用 Element Plus -->
<el-button type="primary">提交</el-button>
<el-tag type="success">已完成</el-tag>
<el-empty description="暂无数据" />

<!-- ❌ 避免自定义基础组件 -->
<button class="custom-btn">提交</button>
```

---

## 8. 路由规范

### 8.1 路由配置结构

```typescript
// src/router/index.ts
import { createRouter, createWebHistory } from 'vue-router'
import MainLayout from '@/components/layout/MainLayout.vue'

const router = createRouter({
  history: createWebHistory(import.meta.env.BASE_URL),
  routes: [
    {
      path: '/',
      component: MainLayout,
      children: [
        {
          path: '',
          name: 'dashboard',
          component: () => import('@/views/dashboard/DashboardView.vue'),
          meta: { title: '仪表盘 - HRM2招聘管理系统' }
        },
        {
          path: 'positions',
          name: 'positions',
          component: () => import('@/views/positions/PositionsView.vue'),
          meta: { title: '岗位设置 - HRM2招聘管理系统' }
        }
        // ...
      ]
    }
  ]
})
```

### 8.2 路由规范要点

| 规范 | 说明 |
|------|------|
| 懒加载 | 使用 `() => import('@/views/{view}/{View}View.vue')` 动态导入 |
| 命名路由 | 使用 `name` 属性，采用 `kebab-case` |
| Meta 信息 | 设置 `title` 用于页面标题 |
| 嵌套路由 | 使用 `MainLayout` 包裹业务页面 |
| 过渡动画 | 路由切换使用淡入淡出过渡 |

### 8.3 页面标题设置

通过路由守卫自动设置：

```typescript
router.beforeEach((to, from, next) => {
  if (to.meta?.title) {
    document.title = to.meta.title as string
  } else {
    document.title = 'HRM2招聘管理系统'
  }
  next()
})
```

---

## 9. 新功能开发流程

以添加新业务模块 `{module}` 为例：

### Step 1: 创建视图目录结构

```
src/views/{module}/
├── {Module}View.vue         # 视图主文件
├── components/              # 视图专属组件（扁平结构）
│   ├── {Module}ListPanel.vue
│   └── {Module}EditForm.vue
└── composables/             # 视图专属 composables（如需要）
    └── use{Module}Editor.ts
```

### Step 2: 创建视图专属 Composable

在 `src/views/{module}/composables/use{Module}Editor.ts` 创建业务逻辑：

```typescript
/**
 * {模块}编辑 composable
 */
export function use{Module}Editor() {
  const loading = ref(false)
  const items = ref<{Module}Data[]>([])
  
  const loadItems = async () => { ... }
  
  return { loading, items, loadItems }
}
```

> 如果该 composable 需要被多个视图使用，则放到 `src/composables/` 中。

### Step 3: 创建视图专属组件

在 `src/views/{module}/components/` 中直接创建组件文件，无需 `index.ts`：

```
src/views/{module}/components/
├── {Module}ListPanel.vue
├── {Module}EditForm.vue
└── {Module}Dialog.vue
```

### Step 4: 创建页面视图

在 `src/views/{module}/{Module}View.vue` 创建页面：

```vue
<template>
  <div class="{module}-view">
    <!-- 页面内容 -->
  </div>
</template>

<script setup lang="ts">
import {Module}ListPanel from './components/{Module}ListPanel.vue'
import {Module}EditForm from './components/{Module}EditForm.vue'
import { use{Module}Editor } from './composables/use{Module}Editor'
// ...
</script>
```

### Step 5: 定义前端类型（如需要）

如果需要扩展 API 类型，在 `src/types/{module}.ts` 创建并在 `src/types/index.ts` 中导出：

```typescript
export type { {Module}Data } from './{module}'
```

### Step 6: 添加路由

在 `src/router/index.ts` 添加路由配置：

```typescript
{
  path: '{module}',
  name: '{module}',
  component: () => import('@/views/{module}/{Module}View.vue'),
  meta: { title: '{模块名称} - HRM2招聘管理系统' }
}
```

### Step 7: 添加导航

在 `src/components/layout/AppSidebar.vue` 添加导航项。

### Step 8: 验证

1. 启动开发服务器：`npm run dev`
2. 访问新页面验证功能
3. 运行类型检查：`npm run type-check`

---

## 附录：常用代码片段

### View 页面模板

```vue
<template>
  <div class="{module}-view">
    <!-- 页面标题 -->
    <div class="page-header">
      <h1 class="page-title">{模块名称}</h1>
      <p class="page-desc">{模块描述}</p>
    </div>

    <!-- 主内容 -->
    <div class="content-area">
      <!-- 内容 -->
    </div>
  </div>
</template>

<script setup lang="ts">
import { onMounted } from 'vue'
import { use{Module}Editor } from './composables/use{Module}Editor'

const { loading, items, loadItems } = use{Module}Editor()

onMounted(() => {
  loadItems()
})
</script>

<style scoped>
.{module}-view {
  display: flex;
  flex-direction: column;
  gap: 24px;
}

.page-header {
  margin-bottom: 8px;
}

.page-title {
  margin: 0 0 8px 0;
  font-size: 24px;
  font-weight: 600;
  color: #303133;
}

.page-desc {
  margin: 0;
  font-size: 14px;
  color: #909399;
}
</style>
```

### Composable 模板

```typescript
/**
 * {模块}管理 composable
 * {功能描述}
 */
import { ref } from 'vue'
import { ElMessage } from 'element-plus'
import { getItems, createItem, deleteItem } from '@/api/sdk.gen'
import type { ItemResponse } from '@/api/types.gen'

export function use{Module}Editor() {
  // 状态
  const loading = ref(false)
  const items = ref<ItemResponse[]>([])

  // 加载列表
  const loadItems = async () => {
    loading.value = true
    try {
      const response = await getItems({ query: { page_size: 100 } })
      items.value = response.data?.data?.items || []
    } catch (err) {
      console.error('加载失败:', err)
      ElMessage.error('加载失败')
    } finally {
      loading.value = false
    }
  }

  // 创建
  const handleCreate = async (data: CreateData) => {
    try {
      await createItem({ body: data })
      ElMessage.success('创建成功')
      await loadItems()
    } catch (err) {
      console.error('创建失败:', err)
      ElMessage.error('创建失败')
    }
  }

  return {
    loading,
    items,
    loadItems,
    handleCreate
  }
}
```

### 组件 Props/Emits 模板

```vue
<script setup lang="ts">
import type { ItemResponse } from '@/api/types.gen'

// Props
const props = defineProps<{
  items: ItemResponse[]
  selectedId: string | null
  loading: boolean
}>()

// Emits
const emit = defineEmits<{
  select: [item: ItemResponse]
  delete: [item: ItemResponse]
  create: []
}>()

// 事件处理
const handleSelect = (item: ItemResponse) => {
  emit('select', item)
}
</script>
```

---

> 📝 **维护说明**：本文档随项目架构演进而更新。如有疑问或建议，请及时反馈。
