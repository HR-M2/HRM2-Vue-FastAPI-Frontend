      ...
      <div v-if="generatedResult" class="result-section">
        ...
        <div class="result-preview">
          <div class="result-item">
            ...
            <span class="value">{{ generatedResult.title }}</span>
          </div>
          ...
          <div v-if="generatedResult.salary_min || generatedResult.salary_max" class="result-item">
            <span class="label">薪资范围：</span>
            <span class="value">
              {{ generatedResult.salary_min }} - {{ generatedResult.salary_max }} K/月
            </span>
          </div>
        </div>
...
const emit = defineEmits<{
  ...
  'apply': [data: Partial<PositionFormData>]
}>()
...
const generatedResult = ref<Partial<PositionFormData> | null>(null)
    ...
    const response = await aiGeneratePositionApiV1AiPositionGeneratePost({
      body: {
        ...
        documents: parsedDocuments.value.length > 0 
          ? parsedDocuments.value.map(d => ({ name: d.name, content: d.content }))
          : undefined
      }
    })
...
.usage-tip :deep(.el-alert__content) {
  font-size: 13px;
  line-height: 1.5;
}
