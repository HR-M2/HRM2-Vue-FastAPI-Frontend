 ...
 * 推送消息类型：
 * - connected: 连接建立
 * - status: 最新任务状态快照（与 /status 字段一致）
 * - heartbeat: 心跳包
 * - error: 异常
 */
export const streamScreeningStatus = <ThrowOnError extends boolean = false>(options: Options<StreamScreeningStatusData, ThrowOnError>) => (options.client ?? client).get<StreamScreeningStatusResponses, StreamScreeningStatusErrors, ThrowOnError>({ url: '/api/v1/screening/{task_id}/stream', ...options });
